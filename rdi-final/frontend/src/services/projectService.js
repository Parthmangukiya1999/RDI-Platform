...
export const getProjectApplications = async (projectId) => {
  const response = await axios.get(
    `${API_URL}/${projectId}/applications`,
    authHeaders()
  );
  return response.data;
};
...
export const updateApplicationStatus = async (projectId, payload) => {
  const response = await axios.put(
    `${API_URL}/${projectId}/application-status`,
    payload,
    authHeaders()
  );
  return response.data;
...
export const removeMemberFromProject = async (projectId, memberUserId) => {
  const response = await axios.delete(
    `${API_URL}/${projectId}/members/${memberUserId}`,
    authHeaders()
  );
  return response.data;
};
