...
export const getProjectReviews = async (req, res) => {
  try {
    ...
    const canView = isOwner || ["admin", "mentor", "company", "expert"].includes(req.user.role);
    if (!canView) {
      return res.status(403).json({ message: "Not authorized to view project reviews" });
    }
    ...
    return res.status(500).json({ message: "Server error while fetching project reviews" });
  }
};

export const getReviewCandidates = async (_req, res) => {
  try {
    const users = await User.find({ role: { $in: ["mentor", "expert", "company"] } })
      .select("name email role")
      .sort({ role: 1, name: 1 });

    return res.status(200).json(users);
  } catch (error) {
    console.error("getReviewCandidates error:", error);
    return res.status(500).json({ message: "Server error while fetching review candidates" });
  }
};

export const assignProjectReviewers = async (req, res) => {
  try {
    const { projectId } = req.params;
    ...
    const isOwner = project.createdBy.toString() === req.user._id.toString();
    const canAssign = isOwner || ["admin", "mentor"].includes(req.user.role);
    if (!canAssign) {
    ...
    for (const item of reviewers) {
      if (!item?.reviewerId || !item?.reviewerRole) continue;
      if (!["mentor", "expert", "company", "peer"].includes(item.reviewerRole)) continue;
    ...
    const reviews = await Review.find({ project: projectId }).populate("reviewer", "name email role").sort({ createdAt: -1 });

    return res.status(200).json({
      message: created.length ? "Reviewers assigned successfully" : "No new reviewers were assigned",
      createdCount: created.length,
      reviews,
    });
  } catch (error) {
    console.error("assignProjectReviewers error:", error);
    return res.status(500).json({ message: "Server error while assigning reviewers" });
  }
};
