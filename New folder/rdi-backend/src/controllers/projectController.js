...
export const recalculateProjectRatingSummary = async (projectId) => {
  ...
  const presentRoles = Object.entries({ mentorAverage, peerAverage, companyAverage, expertAverage })
  ...
  const summary = {
    ...
    isVerified: [mentorAverage, peerAverage, companyAverage, expertAverage].every((score) => score > 0),
...
export const applyToProject = async (req, res) => {
  try {
    ...
    const existingApplication = await Application.findOne({ projectId, applicantId: req.user._id });
...
export const updateApplicationStatus = async (req, res) => {
  try {
    ...
    const application = await Application.findOne({ _id: applicationId, projectId }).populate(
      "applicantId",
      "name email role"
    );
    if (!application) {
    ...
    if (status === "accepted") {
      ...
      if (!alreadyMember) {
        project.members.push({ user: application.applicantId._id, role: "member" });
      }
    ...
    return res.status(200).json({ message: `Application ${status} successfully`, application });
  } catch (error) {
...
export const markProjectCompleted = async (req, res) => {
  try {
    ...
    for (const assignment of assignments) {
      ...
      if (assignments.findIndex((item) => `${item.reviewer.toString()}-${item.reviewerRole}` === uniqueKey) !== assignments.indexOf(assignment)) {
        continue;
      }
